...
class Edge(object):
    ...
    def __eq__(self, other):
        return (self.eid == other.eid and
                self.frm == other.frm and
                self.to == other.to and
                self.elb == other.elb)
    
    def __repr__(self):
        return '(frm={}, to={}, elb={}, eid={})'.format(
            self.frm, self.to, self.elb, self.eid
        )
...
class Vertex(object):
    ...
    def set_bbname(self, bbname):
        self.bbname = bbname
    
    def set_opname(self, opname):
        self.opname = opname
    
    def set_opcode(self, opcode):
        self.opcode = opcode
    
    def set_ori_vid(self, ori_vid):
        self.ori_vid = ori_vid

    def set_glb_vid(self, glb_vid):
        self.glb_vid = glb_vid
...
class Graph(object):
    ...
    def add_vertex(self, 
        ...
        self.vertices[vid].set_modulename(modulename)
        self.vertices[vid].set_opname(opname)
        self.vertices[vid].set_funcname(funcname)
        self.vertices[vid].set_bbname(bbname)
        self.vertices[vid].set_ori_vid(ori_vid)
        self.vertices[vid].set_opcode(opcode)
        self.vertices[vid].set_glb_vid(glb_vid)
        return self

    def add_op_attrs(self, op_attrs):
        self.op_attrs.append(op_attrs)
    ...
    def display(self, support=None):
        """Display the graph as text."""
        display_str = ''
        with open("gspan_output/regular.output", "a+") as f:
            f.write('t # {}\n'.format(self.gid))
            ...
            # for i,attr in enumerate(self.op_attrs):
            #     f.write("a %s %s\n"%(attr[0]["frm_vid"], attr[0]["to_vid"]))
            
            if support:
                f.write('\nSupport: {}\n'.format(support))
                f.write('\n-----------------\n')

        # with open("attrs.output", "a+") as f:
        #     f.write('t # {}\n'.format(self.gid))
        #     for i,attr in enumerate(self.op_attrs):
        #         for a in attr:
        #             f.write("a %d %s %s %s %s\n" %(i, a['frm_vid'], a['to_vid'], a['funcname'], a['bbID']))
