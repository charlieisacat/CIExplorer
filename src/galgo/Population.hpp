...
const CHR<T>& Population<T>::operator()(int pos) const
{
   ...
   if (pos > ptr->popsize - 1) {
      std::cout<<"Error: in galgo::Population<T>::operator()(int), exceeding current population memory."<<"\n";
...
const CHR<T>& Population<T>::operator[](int pos) const
{
   ...
   if (pos > ptr->matsize - 1) {
      std::cout<<"Error: in galgo::Population<T>::operator[](int), exceeding mating population memory."<<"\n";
...
inline void Population<T>::select(int pos)
{
   #ifndef NDEBUG
   if (pos > ptr->popsize - 1) {
      std::cout<<"Error: in galgo::Population<T>::select(int), exceeding current population memory. case 1"<<"\n";
   ...
   if (matidx == ptr->matsize) {
      std::cout<<"Error: in galgo::Population<T>::select(int), exceeding current population memory. case 2"<<"\n";
...
{
   return std::accumulate(curpop.cbegin(), curpop.cend(), 0.0, [](double sum, const CHR<T>& chr)->double{return sum + double(chr->fitness);});
}
